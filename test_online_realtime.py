...
def test_online_real_time_simulation():
    ...
    grid_module = GridModule(
        ...
        time_series=None,
        online=True,
        initial_time_series_value=grid_row,
    )
    ...
    for step in range(steps):
        ...
        microgrid.ingest_real_time_data({'load': load_value, 'pv': pv_value, 'grid': [grid_row]})
        ...
        assert measurements['grid'][0] == pytest.approx(grid_row.tolist())
        np.testing.assert_allclose(grid_module.time_series[step, :], grid_row)

        observations, reward, done, info = microgrid.step({'grid': [0.0]})
        assert 'load' in observations
        assert 'pv' in observations
        assert 'grid' in observations
    ...
    assert len(grid_module.time_series) >= steps
